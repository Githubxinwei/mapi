...
function submitRegister(title, fpCount, saveText, downloadText, isDriverInstall)
{
	...
	if (typeof(Worker) !== "undefined" && isDriverInstall)   
	{
	    ...
	    var bg=document.getElementById("bg");
	 	...
	 	box.style.left=document.documentElement.scrollLeft+50+"px";
	 	// box.style.top=document.documentElement.scrollTop+"px"; //为内容层设置位置
	 	isComp=false;
		 dataInitReg();
	    //关闭页面时，提示保存数据
	...
	$.ajax( {
		...
		success : function(result) 
		{
			...
			if(ret == 0)
			{
				...
				//获取验证指纹
				localStorage.setItem("fpTemplate",JSON.stringify({
					fingerprintData:fpTemplate
				}))
			}
			//成功
			if(ret == 0)
					...
					else
					{
						...
						else
						{
							...
							if(compareRet != "ok")
							{
								...
								//获取录入指纹
								localStorage.setItem("fpTemplateAdd",JSON.stringify({
									fingerprintData:fpTemplate
								}))
							}
							else
...
function fpVerification(title, downloadPrompt, isDriverInstall,context)
{
	...
	if(isDriverInstall)
	{
		...
		if(typeof(Worker) != "undefined")
		{
				...
				comparisonDiv.style.left=55+"px";
