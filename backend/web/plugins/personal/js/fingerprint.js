...
function showRegister2(paramArray)
{
	var hrefStr = "";
	var param = '"'+paramArray[0]+'", "'+paramArray[1]+'", "'+paramArray[2]+'", '+null+'';
	$("#fpRegister2").remove();
	$("#downloadDriver").remove();
	//webservice接口调用成功，说明驱动已经安装
	hrefStr = "<a id='fpRegister2' onclick='submitRegister("+param+", true"+")' title='"+paramArray[3]+"' class='linkStyle'>"+paramArray[3]+"</a>";
	$("#fpRegisterDiv2").append(hrefStr);
}
/**
 * 在页面初始化时，计算指纹数量
 * @author wenxin
...
function showNewDriver(){
    var hrefStr = "<a id='downloadDriver' href='/plugins/zkbioonline/zkbioonline.exe' title='下载新驱动'>下载新驱动</a>";
...
function submitRegister(title, fpCount, saveText, downloadText, isDriverInstall)
{
	...
	if (typeof(Worker) !== "undefined" && isDriverInstall)   
	{
		...
		box.style.top=Math.abs(parseInt((document.documentElement.clientHeight-box.offsetHeight)/2))+document.documentElement.scrollTop+"px";
...
//		showImage(context, "${base}/base/images/base_fpVerify_clearImage.png", "clearForVerify");
	...
	{
		showImage(context, "/plugins/privateLesson/image/base_fpVerify_clearImage.png", "clearForRegister");
	}
