...
class Sms extends Model
{
     ...
     * @inheritdoc
     */
    public function setSmsTemplate($to, $cardName)
    {
        $var = ['content' => $cardName ,'venue'=>'我爱运动大上海店' ,'startdate'=>'1月8号'];
        $var = json_encode($var);
        $this->vars = $var;
        $this->to = $to;
        $this->timestamp = time();
...
//        $path == "47.93.202.248"
        ...
        if($path == 'http://memberapi.xingfufit.cn' || $path == 'memberapi.xingfufit.cn'){
       ...
       if(is_array($smsInfo['templates']) && !empty($smsInfo['templates'])){
           ...
           switch($type)
           {
               ...
               case '11'         :
                   ...
                   break;
               case '12'         :
                   $dataInfo     = str_replace(['@var(content)','@var(venue)','@var(startdate)'],[$varData['content'],$varData['venue'],$varData['startdate']],$hh);//YtV4L 柜子到期提醒
                   break;
               default;
           }
           return $dataInfo;
    ...
    public function accessContent($url)
    {
        ...
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0); // 对认证证书来源的检查
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 2); // 从证书中检查SSL加密算法是否存在
        // .net 获取不到用户请求浏览器加判断运算   修改于 2018-01-02
...
//        print '<pre>';print_r(json_decode($tmpInfo,true));die();
     ...
     * @param $info
     */
    public function memberCharge($to)
    {
        $this->setMemberCharge($to);
        $url = $this->url;
...
//            "project"=>'TlIkY',
     ...
     * @inheritdoc
     */
    public function setMemberCharge($to)
    {
        $var = [];
        $var = json_encode($var);
        $this->vars = $var;
     ...
     * @param $course
     */
    public function sendClassInfo($to,$course,$info)
    {
        $this->setNote($to,$course,$info);
        $url = $this->url;
     ...
     * @param $name     //内容
     */
    public function setComeInMaiBuCode($to,$name,$code)
    {
        $this->setSmsTemplate($to,$name);
        $url = $this->url;
        $post_data = array (
            "appid" => $this->appid,
            "to" => $this->to,
        ...
        $this->commonData($url,$post_data,'12','');   //发送
    }
    /**
     * @迈步-办卡-发送短信
     * @create lihuien
