...
var app = angular.module('App',['starter.services']).controller('goBuyCardCtrl',function($timeout,$http,$scope,$rootScope,$interval,dialogsManager,$location){
    //扇叶窗
    $(function(){
    ...
    $scope.confirmRes  = function(){
        // var birthday = $('#date').val().replace(/\//g, "-");
        ...
        if($scope.venueId == undefined || $scope.venueId == null || $scope.venueId == ''){
            ...
            if(!$scope.submitButtonFlag){
                $http.get('/purchase-card/get-mobile-info?mobile='+ $scope.mobile +'&companyId='+$scope.submitData().companyId+'&venueId='+$scope.submitData().venueId).then(function(response) {
                    if (response.data.status == 'error') {
                        $scope.submitButtonFlag = false;
                        $scope.showMessage = function () {
                            dialogsManager.showMessage("该用户已登记过，不能重复登记!");
                        }
                        $scope.showMessage();
                        return;
                    }else if(response.data.status == 'success'){
                        if($scope.submitData().idCard != null) {
                            $http.get('/purchase-card/member-details?MemberIdCard=' + ($scope.submitData().idCard) + "&MemberId=" + null+"&venueId="+$scope.submitData().venueId).then(function (result) {
                                if (result.data != '' || result.data.length > 0) {
                                    $scope.showMessage = function () {
                                        dialogsManager.showMessage("该证件号已经存在");
                                    }
                                    $scope.showMessage();
                                    $scope.submitButtonFlag = false;
                                    return;
            ...
            //             $scope.submitButtonFlag = false;
            //             $scope.showMessage = function () {
            //                 dialogsManager.showMessage("该用户已登记过,不能进行重复登记!");
            //             }
            //             $scope.showMessage();
            ...
            //                     // $scope.showMessage();
            //                     $timeout(function () {
            //                         window.location.href = '/purchase-card/success' + '?time=' + ((new Date()).getTime())
            //                     }, 100)
            //                 } else {
            //                     $scope.submitButtonFlag = false;
            //                     $scope.showMessage = function () {
    ...
    $scope.postSubmit = function () {
        $http({
            url: '/purchase-card/register-potential',
            method: 'POST',
            data: $.param($scope.submitData()),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        }).then(function (result) {
            if (result.data.status == 'success') {
                ...
                $scope.showMessage();
                // location.href = '/purchase-card/complete' + '?time=' + ((new Date()).getTime());
                location.href = '/purchase-card/finish' + '?time=' + ((new Date()).getTime())
        ...
        if(!($pattern.test($scope.mobile))){
            ...
            $scope.timePromiseCode = $interval(function(){
                if($scope.codeSecond<=0){
                    ...
                    $scope.paracont = $scope.codeSecond + "秒后重新获取";
                    $scope.disabled =true;
                    $scope.codeSecond--;
                }
            },1000,100);
...
}).controller('registerSuccessPage',function($scope,$http,$rootScope,$location){
    //会员登记成功页面
