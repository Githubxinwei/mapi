...
use \backend\components\payment\lib\WxPayRefund;
use backend\components\payment\lib\WxPayConfig;
use \backend\components\payment\lib\WxPayApi;
//初始化日志
$logHandler= new \backend\components\payment\example\CLogFileHandler("../logs/".date('Y-m-d').'.log');
$log = \backend\components\payment\example\Log::Init($logHandler, 15);
...
if(isset($_REQUEST["transaction_id"]) && $_REQUEST["transaction_id"] != ""){
    ...
    $input->SetOut_refund_no(\backend\components\payment\lib\Config::MCHID.date("YmdHis"));
    $input->SetOp_user_id(\backend\components\payment\lib\Config::MCHID);
