<?php
namespace backend\components\appPay\lib;
...
class WeEncryption {
	...
	private function __construct() {
		if ((isset($appid) && is_string($appid)) && (isset($mch_id) && is_string($mch_id))) {
			$this->appid   =  AppConfig::APPID;
			$this->mch_id  = AppConfig::MCHID;
	...
	private function setSendData($data) {
		...
		$data = sprintf($this->sTpl, $this->appid,$this->attach, $body, $this->mch_id, $nonce_str, $this->notify_url, $out_trade_no, $spbill_create_ip, $total_fee, $trade_type, $sign);
		return $data;
	}
...
//		$postXml = $GLOBALS["HTTP_RAW_POST_DATA"];	//接受通知参数；
   ...
   <sign>%s</sign>
    ...
    /**
     * 发送下单请求；
     * @param  Curl   $culocalhost/v3/api-wechat-pay/wechat-loginrl 请求资源句柄
     * @return mixed       请求返回数据
     */
    public function MiniSendRequest(Curl $curl, $data) {
        $data = $this->formatMiniData($data);
        $url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
        $curl->setUrl($url);
        $content = $curl->execute(true, 'POST', $data);
        return $content;
     ...
     * @param
     */
    public function formatMiniData($data)
    {
        $xml_data = "<xml>
		<appid><![CDATA[%s]]></appid>
		<body><![CDATA[%s]]></body>
        ...
        $xml_data = sprintf($xml_data, $appid, $body, $mch_id, $nonce_str, $notify_url, $openid, $out_trade_no, $total_fee, $trade_type, $spbill_create_ip, $sign);
        return $xml_data;
    }
}