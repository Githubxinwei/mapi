...
class UrlPager extends Widget
{
    ...
    public $jsPageFun = 'replacementPages';
    ...
    {
        if(isset($jsFun) && $jsFun){
            $this->jsPageFun = $jsFun;
        }
...
//        return json_encode($this->firstPageLabel);
        if ($firstPageLabel !== false) {
            $buttons[] = $this->renderPageButton($firstPageLabel, 0, $this->firstPageCssClass, $currentPage <= 0, false);
//            return json_encode($firstPageLabel);
        }
...
//        var_dump($pageCount);die();
        $ulHtml      = Html::tag('ul', implode("\n", $buttons), $this->options);
        $currentPage = $currentPage + 1;
        $spanHtml    = Html::tag('div',Html::tag('h6','共'.$pageCount.'页'.' '."当前<b>".$currentPage.'</b>页'.' '.'共'.$totalCount.'条'));
        $ulHtml      = $ulHtml.$spanHtml;
        return $ulHtml;
    }
    ...
    protected function renderPageButton($label, $page, $class, $disabled, $active)
    {
        ...
        $linkOptions['ng-click']   = $this->jsPageFun."('".$this->pagination->createUrl($page)."')";
